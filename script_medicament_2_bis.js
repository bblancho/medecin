...
function addElement(tabElements,baliseParent) {
  ...
  let element , titre ,id, tabObjetPresentation, tabObjetComposition ;

  for (let j=0 ; j < tabElements.length ; j++) {
    element = tabElements[j] ;
    titre = element.titre ; 
    id = element.id ;
    tabObjetPresentation  = element.presentations ;
    tabObjetComposition   = element.compositions ;

    //console.log(tabObjetComposition) ;
    ...
    // Infos présentation
      ...
      for (let i = 0; i < tabObjetPresentation.length ; i++) {
        if(tabObjetPresentation[i] != null){
          let li = document.createElement('li') ;
          let text = document.createTextNode( tabObjetPresentation[i] ) ;
          li.appendChild(text);
          ul.appendChild(li);
        }
      }
    ...
    // Infos composition
      ...
      blocComposition.appendChild(ul);
      blocComposition.appendChild(separateur);
  ...
  .then( response => {
    if (response.ok) {
      ...
      .then( tabMedicaments => { // On traite les données
        ...
        //console.log(tabeObjetsMedicament);
